...
def trainModel(
    ...
    # init as tensors so .item() is always valid
    ...
    for epoch in range(epochs):
        for phase in ["train", "val"]:
            ...
            # scalar tracking vars as floats
            loss = 0.0
            ema_loss = 0.0
            ema_l1 = 0.0
            ema_a_recall = 0.0
            ...
            for i, (batch_features, targets) in enumerate(loop):
                ...
                # main loss
                train_loss: torch.Tensor = criterion(outputs, targets) / NUM_ACCUMULATION_STEPS
                ...
                if reinforce:
                    ...
                    if phase == "train":
                        acceptor_acc = torch.nanmean(
                            ...
                            / torch.sum(targets_full[:, 1, :] > 0, dim=1)
                        )
                        acceptor_acc_avg = (
                        ...
                        donor_acc = torch.nanmean(
                            ...
                            / torch.sum(targets_full[:, 2, :] > 0, dim=1)
                        )
                        donor_acc_avg = (
                            donor_acc * multiplier + donor_acc_avg * (1 - multiplier)
                        )
                ...
                with torch.no_grad():
                    ...
                    a_recall = torch.nanmean(
                        (out_argmax[target_argmax == 1] == 1).type(torch.float32)
                    )
                    d_recall = torch.nanmean(
                        (out_argmax[target_argmax == 2] == 2).type(torch.float32)
                    )
                ...
                if phase == "val":
                    if lowValidationGPUMem:
                        ...
                        targets_list.extend(
                            np.expand_dims(targets.cpu().numpy(), 0)
                        )
                        outputs_list.extend(np.expand_dims(outputs_np, 0))
                    else:
                        outputs = outputs.detach()
                        outputs_list.extend(outputs.unsqueeze(0))

                # update running loss and EMA stats
                loss = NUM_ACCUMULATION_STEPS * train_loss.item() + loss
                loop.set_description(
                    "Epoch ({}) {}/{}".format(phase, epoch + 1, epochs)
                )
                n_steps_completed += 1
                ...
                if i == 0:
                    ...
                    if ~d_recall.isnan():
                        # BUGFIX: use d_recall here, not a_recall
                        ema_d_recall = d_recall.cpu().numpy()
                else:
                    ema_loss = (
                        NUM_ACCUMULATION_STEPS * train_loss.item() * multiplier
                        + ema_loss * (1 - multiplier)
                    )

                    if ~a_recall.isnan():
                        ema_a_recall = (
                            a_recall.cpu().numpy() * multiplier
                            + ema_a_recall * (1 - multiplier)
                        )
                    if ~d_recall.isnan():
                        ema_d_recall = (
                            d_recall.cpu().numpy() * multiplier
                            + ema_d_recall * (1 - multiplier)
                        )

                if reinforce:
                    if i == 0:
                        ema_reinforce_loss = (
                            NUM_ACCUMULATION_STEPS * reinforce_loss.item()
                        )
                    else:
                        ema_reinforce_loss = (
                            NUM_ACCUMULATION_STEPS
                            * reinforce_loss.item()
                            * multiplier
                            + ema_reinforce_loss * (1 - multiplier)
                        )
                    loop.set_postfix(
                        loss=ema_loss,
                        r_a=acceptor_acc_avg.item(),
                        r_d=donor_acc_avg.item(),
                        r_loss=ema_reinforce_loss,
                        a_r=ema_a_recall,
                        d_r=ema_d_recall,
                    )
                else:
                    loop.set_postfix(
                        loss=ema_loss, a_r=ema_a_recall, d_r=ema_d_recall
                    )

            # end for i,(batch_features, targets)
            ...
            if phase == "val":
                if lowValidationGPUMem:
                    ...
                    targets = (
                        torch.transpose(torch.vstack(targets_list), 1, 2)
                        .cpu()
                        .numpy()
                    )
                    outputs = (
                        torch.transpose(torch.vstack(outputs_list), 1, 2)
                        .cpu()
                        .numpy()
                    )
            ...
            if phase == "val":
                (
                    ...
                    np.array(Y_true_acceptor),
                    np.array(Y_pred_acceptor),
                    np.array(Y_true_donor),
                    np.array(Y_pred_donor),
                )
                if continous_labels:
                    Y_true_acceptor = np.round(Y_true_acceptor, decimals=0)
                    Y_true_donor = np.round(Y_true_donor, decimals=0)
                print("\n\033[1m{}:\033[0m".format("Acceptor"))
                acceptor_val_results = print_topl_statistics(
                    Y_true_acceptor, Y_pred_acceptor
                )
                print("\n\033[1m{}:\033[0m".format("Donor"))
                donor_val_results = print_topl_statistics(
            ...
            if verbose == 1:
                print(
                    "epoch: {}/{}, {} loss = {:.6f}".format(
                        epoch + 1, epochs, phase, loss
                    )
                )

            # Checkpoint logic:
            # - If we are running validation (skipValidation=False), only save when
            #   validation loss improves.
            # - If skipValidation=True, we save the last-epoch weights as before.
            if phase == "val":
                if loss < best_val_loss:
                    best_val_loss = loss
                    torch.save(model.state_dict(), fileName)
            elif skipValidation:
                # No validation phase: always save (same behavior as before)
                torch.save(model.state_dict(), fileName)
